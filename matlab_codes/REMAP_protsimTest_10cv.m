...
%    [auc,aupr]=AUROC(TestCompressed,P);
	 [~,~,~,auc]=perfcurve(TestCompressed(:),P(:),1);%auc for default test
%	 [~,~,~,aupr]=perfcurve(TestCompressed(:), P(:), 1, 'xCrit', 'reca', 'yCrit', 'prec');
    ...
    [~,~,~,auc_new]=perfcurve(TestCompressed(:),P_new(:),1); %auc for new prot-sim
